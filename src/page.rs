...
impl<'a> Page<'a> {
  fn new(path: &str, posts: &'a Vec<Post>, partials: &'a Vec<Partial>) -> Page<'a> {
    Page {
      output_html: None,
      source_content: None,
...
mod tests {
  ...
  mod new {
    ...
    fn should_create_a_page() {
      let posts = vec![];
      let partials = vec![];
      let mut page = Page::new("test/assets/pages/simple.handlebars", &posts, &partials);
      page.process();
    ...
    fn should_produce_html_from_the_template() {
      ...
      page.process();
      assert_eq!(page.output_html.unwrap(), "<div>Hello world</div>");
    }
    ...
    fn it_should_interpolate_posts_in_the_html() {
      ...
      let mut page = Page::new("test/assets/pages/posts.handlebars", &posts, &partials);
      page.process();
      assert_eq!(page.output_html.unwrap(), "<div>Title - Badger</div>");
    }
    ...
    fn it_should_be_able_to_render_other_partial_templates() {
      ...
      let mut page = Page::new("test/assets/pages/partial.handlebars", &posts, &partials);
      page.process();
